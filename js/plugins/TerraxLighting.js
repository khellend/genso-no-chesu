 ...
 * 要激活某个区域中的脚本，请执行以下操作:
 * 1. 放置一个事件开关到地图中。
 * 2. 在“Note”栏位(地图名称旁)键入以下文字:
 * Light 250 #FFFFFF
 * - 灯光激活脚本
 * - 250 是物体的亮度范围
 * - #FFFFFF 为是灯光颜色(本例中为白色)
 * 3. You're done, its that simple.
 *
 * 您可以为亮度和方向添加两个可选命令
 * Light 200 #FFFFFF B50 增加50%的亮度。值在0到99之间。
 * Light 200 #FFFFFF D1 将会为放置在墙上的光源提供半个光圈。
 * 1. 对于北边墙上的灯，光线将朝下。
 * 2. 东墙上的灯，是朝西的。
 * 3. 对于南墙上的灯，光线将朝北。
 * 4. 对于西墙上的灯，光将面向东方。
 *
 * 要改变游戏中的玩家半径，可以使用以下插件命令:
 * Light radius 200 #FFFFFF  (改变半径和颜色)
 * 如果你想随着时间慢慢改变玩家的半径(就像一个垂死的火炬)
 * 使用命令 'Light radiusgrow 200 #FFFFFF'
 * 你可以通过添加以下来改变玩家亮度:
 * 'Light radius 200 #FFFFFF B70' (亮度值在0到99之间，默认为0)
 *
 * 要在游戏中打开和关闭光源，请执行以下操作:
 * 将光源设置为正常的默认值 :  Light 250 #FFFFFF 和一个额外的数字
 * 所以它变成了 'Light 250 #FFFFFF 1'
 * (如果你使用可选的亮度和方向，把这个放在它们后面 (Light 250 #FFFFFF B50 D2 1)
 * 要打开此灯，请使用插件命令: 'Light on 1'.
 * 插件命令还将触发目标灯上的独立开关“D”。
 * 要关闭灯使用插件命令 : 'Light off 1'.
 * 您可以通过选项或插件命令
 *  'Light switch reset' 手动重置每个灯光的开关
 * 您还可以使用参数中定义的kill-selfswitch关闭电灯。
 * 您可以更改具有id的光源的颜色。使用插件调用
 * 'Light color 1 #FF0000'
 *
 * 您可以使用下面的note-tag让灯光颜色自动循环
 * Light cycle #FF0000 50 #00FF00 50 #0000FF 50
 * 这将使颜色循环从红色到绿色到蓝色，每50毫秒。
 * 最少2种颜色，最多4种颜色。
 *
 * Replacing the 'Light' keyworld with 'Fire' will give the lights a subtle flicker
 * 您可以使用plugin命令配置火焰效果 'SetFire 7 10'
 * 其中7是半径的变化，10是颜色从红色到黄色的变化。
 * 
 * 要完全关闭脚本使用 : 'Light deactivate'
 * 要再次打开它，请使用该命令: 'Light activate'
 *
 * 要在地图上激活昼夜循环，可以在事件记录或地图记录中
 * 加入一个带有“DayNight”的触发器。
 * 插件命令'Daynight speed 10'改变速度。
 * Speed 10表示在游戏中10秒就可以通过一个小时(可能很快)
 * 插件命令'Daynight hour 16 30 '将时间设置为16:30小时
 * 每个小时都有自己的颜色值。
 * 插件命令'Daynight color 0 #222222'将0:00小时更改为颜色值'#222222'
 * 您可以使用插件命令“Daynight add 8 30”来添加时间(这将添加8小时30分钟)
 *
 * 如果你想利用一天的时间来触发效果(比如天黑时开灯)
 * 你可以使用参数 'Save DaynightHours','Save DaynightMinutes','Save DaynightSeconds'
 * 默认值是0，这意味着它是关闭的。
 * 如果您将其设置为一个值，例如5，它将在游戏变量5中存储昼夜数值。
 * 然后，您可以使用该变量来触发灯光。
 * 要帮助同步/调试时间系统，可以使用'daynight debug'来显示当前时间。
 * 如果你想去“外星世界”之类的地方，你可以用它来改变一天的小时数
 * 脚本命令'daynight hoursinday 48'(对于一天中的48小时，不要忘记填写小时值)
 *
 * 作为昼夜循环的一种选择，你可以使用色调系统。如果你想在白天/晚上使用另一个插件
 * tint选项可能是最好的选择。
 * 插件命令“Tint set #333333”将使房间不那么黑。
 * 插件命令“Tint fade #777777 5”将使颜色从当前颜色渐变为最新颜色
 * 数字(5)是淡出的速度，were 1是快速淡出，而20是非常慢的淡出。
 * 如果某个区域有昼夜循环系统，则禁用着色系统。
 *
 * 使用手电筒效果 'Flashlight on 8 12 #FFFFFF 3' and 'Flashlight off'
 * 参数是可选的(8=波束长度，12=波束宽度，#FFFFFF=颜色，3=波束密度)
 * 事件也可以使用闪光灯效果。在备注标签中使用'Flashlight 8 12 #888888 1 2' 。
 * 其中8为手电筒光束的长度，12为光束的宽度。最后一个数字是可选的，
 * 可以用来打开或关闭NPC的手电筒，并设置光束的方向
 * 如果事件没有移动(1=上、2=右、3=下、4=左)，则默认为向下。
 *
 * 图块组和区域灯光设置
 * 要在不使用事件的情况下创建光源，可以使用以下插件命令。
 * TileLight 1 ON #FFFFFF 150  将在所有带有tile-tag 1的tile上 (color #FFFFFF radius 150)创建一个光源。
 * TileRegion 1 ON #FFFFFF 150 将在所有具有区域1的贴片上创建一个光源。
 ...
 * 关闭TileLight 1将再次关闭tile-tag 1上的灯
 * TileRegion 1关闭将再次关闭1号区域的灯
 * 图块组火和区域火的工作原理与TileLight相同，但具有火焰效果。
 * TileGlow和RegionGlow的工作原理与TileLight相同，但具有轻微的脉冲效果。
 * 确保您的地图中至少还有一个带有灯光的事件，否则脚本将不会运行。
 *
 * 块和区域块设置
 * 要阻止某些瓦片(例如屋顶)上的灯光，可以使用以下插件命令。
 * TileBlock 1 ON #000000  将用瓷砖标签1挡住瓷砖上的光。
 * RegionBlock 1 ON #000000 将阻止与1号区域瓷砖上的灯。
 * TileBlock 1 OFF and TileRegion 1 OFF 再次关闭阻塞。
 * 要使光线变暗但不能完全阻挡光线，可以使用稍高的颜色设置(#333333)。
 * 此函数不进行光线跟踪。如果玩家lightradius更大，那么右边的方块就会出现在另一边。
 * 为了达到最好的效果，保持光传统比块部分小一点。
 * 对于高级用户，如果希望或多或少地阻塞tile，可以执行以下操作
 * RegionBlock 1 ON #000000 shape xoffset yoffset width height
 * RegionBlock 1 ON #000000 1 20 20 10 10   -> 这将阻塞一个从20,20开始的盒子，宽度和高度分别为10,10
 * RegionBlock 1 ON #000000 2 20 20 10 10   -> 这将阻塞一个椭圆形，从20开始，20的xradius 10和yradius 10
 *
 * 您可以使用以下插件调用为爆炸或法术创建短时间的灯光效果。
 * 插件命令: effect_on_event 003 200 #FFAAAA 50
 * -> 将创建一个id为003，半径为200，颜色#FFAAAA，持续时间为50帧的事件的lightglobe
 * 插件命令: effect_on_xy 560 500 50 #FFAAAA 25
 * -> 将创建一个坐标560.500，半径50，颜色#FFAAAA，持续25帧的光球
 * 一些特殊效果可以添加，说明在手册中
 * Terrax照明系统与Moghunter时间系统兼容，详情请参阅read-me。
 *
